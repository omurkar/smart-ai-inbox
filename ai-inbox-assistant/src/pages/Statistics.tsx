...
import { ArrowLeft, PieChart, TrendingUp, Inbox, CheckCircle2, Info, X, Zap, Clock, Coffee, Archive } from 'lucide-react'

export function Statistics() {
  const nav = useNavigate()
  const { emails, analysisById, archivedIds } = useMailStore()
  
  // NEW: State to control the visibility of the info modal
  ...
  const stats = useMemo(() => {
    ...
    return { high, medium, low, archived, unanalyzed, total }
      ...
      <div className="mx-auto w-full max-w-5xl px-6 py-10">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-slate-400">A breakdown of your email categories and productivity metrics.</p>
          </div>
          
          {/* NEW: Button to open the info modal */}
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_1fr]">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col items-center justify-center">
            {totalAnalyzed === 0 ? (
              ...
              <>
                <div 
                  className="h-64 w-64 rounded-full shadow-2xl shadow-black/50 transition-transform hover:scale-105"
          ...
          <div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="mt-6 space-y-6 max-h-[60vh] overflow-y-auto pr-2">
              <p className="text-sm text-slate-300">
                The SmartInbox AI uses advanced language models to read the context of your emails. If the AI is temporarily unavailable, it falls back to a strict keyword-scanning heuristic to instantly classify your mail into the following buckets:
              </p>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-indigo-300">High Priority</h3>
                  <p className="mt-1 text-sm text-slate-400">
                    Emails requiring immediate action, addressing critical issues, or involving financial matters.
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {['urgent', 'asap', 'today', 'eod', 'past due', 'overdue', 'invoice', 'payment', 'action required', 'critical', 'emergency', 'deadline', 'immediate', 'important', 'escalation', 'failed', 'error', 'alert', 'attention', 'blocked', 'issue'].map(k => (
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    Standard business workflows, scheduling, meeting requests, and collaborative discussions.
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {['meeting', 'schedule', 'calendar', 'update', 'review', 'follow up', 'proposal', 'feedback', 'question', 'reminder', 'discuss', 'opportunity', 'invitation', 'status', 'request', 'sync', 'check-in'].map(k => (
                      <span key={k} className="rounded-md bg-amber-500/10 px-2 py-0.5 text-[11px] text-amber-200 border border-amber-500/20">{k}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-emerald-300">Low Priority</h3>
                  <p className="mt-1 text-sm text-slate-400">
                    Newsletters, marketing emails, automated confirmations, and general updates lacking the urgency keywords mentioned above.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    Emails you have manually processed and moved out of your active feed by clicking the "Archive" button. These are removed from your active Inbox label in Gmail but are preserved for record-keeping.
                  </p>
                </div>
              </div>
